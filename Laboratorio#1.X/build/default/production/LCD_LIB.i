...
void _write_nibble(uint8_t nibble){
    if( (nibble & 0x01) == 0) PORTF &= ~(1<<4); else PORTF |= (1<<4);
    if( (nibble & 0x02) == 0) PORTF &= ~(1<<5); else PORTF |= (1<<5);
    if( (nibble & 0x04) == 0) PORTF &= ~(1<<6); else PORTF |= (1<<6);
    if( (nibble & 0x08) == 0) PORTF &= ~(1<<7); else PORTF |= (1<<7);
...
void _send_byte(uint8_t value, uint8_t mode){
    ...
    switch(mode){
        case 0: PORTF &= ~(1<<0); break;
        case 1: PORTF |= (1<<0); break;
    }
